...
-- 9:
CREATE USER 'mnavarro'@'localhost'
  IDENTIFIED BY '9876';
GRANT ALL
  ON *.*
  TO 'mnavarro'@'localhost'
  WITH GRANT OPTION;
  
-- 10:
SELECT host, user FROM user WHERE user='mnavarro';
SELECT host, db, user FROM db WHERE user='mnavarro';
SHOW GRANTS FOR 'mnavarro'@'localHost';
